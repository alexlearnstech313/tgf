...
This discipline applies to the workflow itself: per-stage methodology cited in `docs/WORKFLOW.md` §3.
...
`docs/WORKFLOW.md` §3 specifies the authoritative methodology grounding this stage.
...
Total catalog target: **~75 skills** across all phases (consolidated from ~80 per `DEC-2026-05-19-008` — architectural cluster 5→2, CIA triad 3→1). Adopter projects see only the subset their project mode + scope warrants. The three always-on skills (`code-quality/`, `security-core/`, `continuity/`) are detailed in §6; every other skill loads conditionally on contextual triggers gated by project mode (§4).

For the full enumerated catalog — always-on, activity, universal security, operations/quality, compliance, meta, and project-specific skills, each with its description and build status — see `docs/SKILL-INDEX.md`. That detail lives outside this contract so the per-skill enumeration doesn't load into every session's context (same rationale as the §15–§22 → `docs/ARCHITECTURE.md` split).
