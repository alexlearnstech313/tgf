...
### §3.17 Target 17 — `docs/ARCHITECTURE.md` (Audit completed 2026-05-28 — Build Step 6; foundational doc, ALL FOUR agents)

**Audit context:** WS4 Build Step 6 Target 17 (commit 19/22). Decision H all-four-agents, focused lenses. The §15-§22 extended architecture CLAUDE.md §15-§22 point into.

**Activity logs:** code-reviewer `ba30f5b1-...`; security-auditor `.tgf/state/agent-activity/security-auditor/f12afb58-2a8c-4906-b1be-230a7e6ddb3a.json`; red-team `.tgf/state/agent-activity/red-team/d73e5089-e869-4688-abd4-86cf2d3a101e.json`; holistic `3a7a9470-...`

**Routed to ERROR-LOG.md (1 consolidated HIGH):**
- **ERR-2026-05-28-014 (High)** — §18/§20 do NOT specify the enforcement gates ERR-010/011/012/013 were deferred to (the 3 universal hooks don't cover hard-refusal-invariant matching, review-evidence, or tier-floor; §20 reproduces tier self-assignment; §19 cost framing pushes toward under-dispatch; all hook scripts deferred to Phase 12). **Consolidates F-RT-ARCH-01 + F-RT-ARCH-02; removes the "runtime may mitigate" hedge from ERR-010/011/012/013 — those four are real end-to-end today, mitigated only by persona discipline.** The shipped M8 gate is the fix template (extend, don't invent).

**Routed here (Medium/Low):**

| ID | Severity | Target | Description | Remediation hint | Priority hint |
|---|---|---|---|---|---|
| F-CR-ARCH-02 / F-SA-ARCH-04 | Medium | ARCHITECTURE.md §21 L380 | §21 presents `.tgf/evolution/observations/` as authoritative, but DEC-2026-05-19-007 Clause 9 superseded it with `.claude/agent-memory/{agent-name}/MEMORY.md`. §21 is the doc DEC-007 Clause 9 cites — they contradict. LLM04 mitigations point at the wrong data store. (proposals/ workflow unaffected.) | Amend §21 L380 to the agent-memory path, note the supersession inline (like §18 L203 does for DEC-005). | stale content (DEC-superseded) |
| F-SA-ARCH-01 | Medium | ARCHITECTURE.md §20 L316-320 | §20 presents the per-role `tools:` restriction as enforced least-privilege; contradicts **open ERR-2026-05-25-002** (documentation+persona-backed, NOT platform-validated — Decision M Q4c undocumented). Overstates the primary LLM06 control's maturity. | Add a maturity caveat pointing to ERR-2026-05-25-002; frame persona discipline as the currently-observed layer. | overstated security maturity |
| F-SA-ARCH-02 | Medium | ARCHITECTURE.md §17 L108-112 | §17's citation-verification summary OMITS the M9 memory-confirmation-gap — the load-bearing rule from the commit 4/12 incident (RESEARCH-SECURITY §8.7, DEC-2026-05-26-011 Clause 3). A reader of §17 alone could satisfy "two sources" with memory + one fetch (the defeated pattern). | Add: cross-source corroboration requires ≥1 source structurally independent of AI prior knowledge; memory + one fetch = one source, not two (M9). | §17 omits the M9 anchor |
| F-RT-ARCH-03 | Medium | ARCHITECTURE.md §21 L383-433 | LLM04 poisoning: trigger-criteria proposals NARROWING a security skill's applies-when can be pushed past the count+distinctness threshold by a patient cross-session campaign (anti-poisoning checks tuned for volume-spikes, not slow campaigns). Bounded by human-review backstop → Medium. No security-delta gate. | Add a security-delta classification: trigger-narrowing proposals on security-* skills routed to a stricter review bar regardless of count. | self-evolution poisoning-resistance |
| F-RT-ARCH-04 | Medium | ARCHITECTURE.md §19 L238-245 | §19 path-based pre-filter is fail-OPEN-for-security: a security skill whose paths-include is too narrow is dropped BEFORE its imports/operations/data-flows conditions evaluate (the conditions that catch security-relevant code in unexpected locations — crypto in /utils/, auth in /helpers/). Presented purely as cost optimization, no security caveat. | security-* skills must not be droppable by the path pre-filter alone (always proceed to full applies-when, or default paths broad). Cross-ref CLAUDE.md §4 "when in doubt, load". | activation fail-open |
| F-CR-ARCH-01 / F-HR-ARCH-01 | Low | ARCHITECTURE.md §17 L94 | docs/-path-rot CONFIRMED once (docs/DECISIONS.md; file at root) vs 2 correct bare uses (L399/517) → ~90% clean, unambiguous typo. **Path-rot is now CLAUDE.md + this 1 ARCHITECTURE.md line — NOT systemic (WORKFLOW.md clean).** | Change to bare DECISIONS.md; fold into the F-HR-CLAUDE-01 sweep. | path-prefix consistency |
| F-CR-ARCH-03 | Low | ARCHITECTURE.md §21/§22 | /tgf:review-evolution referenced 3× but absent from CLAUDE.md §10 command registry (commands are Phase 11+). | Add to §10, or soften to "(Phase 11 will provide...)". | command-registry completeness |
| F-CR-ARCH-04 | Low | ARCHITECTURE.md §17 L128 | DESIGN-RATIONALE.md referenced but doesn't exist. | Drop or qualify as planned / name the existing design-notes docs. | dangling reference |
| F-SA-ARCH-03 | Low | ARCHITECTURE.md §18 L152-156 | §18 "Exit 2 blocks" generic claim doesn't surface PostToolUse-cannot-block (RESEARCH-SECURITY §5.1 two-stage model). | Note block capability is event-dependent (PreToolUse/Stop block; PostToolUse records+warns). | hook-semantics precision |
| F-HR-ARCH-02 | Low | ARCHITECTURE.md §20 L278-291 | §20 describes "seven subagent roles" in present tense, but only 4 review agents + orchestrator are shipped; Researcher/Implementer/Verifier have no agent files (unflagged, unlike §18/§22's honest deferrals). | Add a §20 clause flagging the 3 role definitions land with the Phase 11 orchestration meta-skill. | calibrated-confidence on build state |
| F-HR-ARCH-03 | Low | ARCHITECTURE.md §20 L280/L372 | §20 lacks a forward pointer to WORKFLOW.md §4 (where the role output schemas live) — one-directional cross-ref. | Add a forward pointer making §20+CLAUDE.md §6+WORKFLOW.md §4 the explicit 3-part preload authority. | cross-ref navigability |

**⭐ §9-preload AUTHORITY clarified (holistic):** §20 is the authority for subagent role descriptions/scoping but NOT the §9-preload mechanism (never mentions skills: frontmatter / CLAUDE.md §6 / WORKFLOW.md §4). The 7-skill WS5 §9-preload fix should align to a **THREE-part authority: §20 (role boundaries) + CLAUDE.md §6 (always-on set) + WORKFLOW.md §4 (per-role schema/preload binding).** All 12 distinct preloaded skills across the 5 shipped agents verified to exist as skills/ dirs.

**docs/-path-rot watch result:** confirmed CLAUDE.md (§7/§11, systemic) + ARCHITECTURE.md (L94, single instance); WORKFLOW.md clean. Re-confirm DECISIONS.md + RESEARCH-SECURITY.md (Targets 18-19).

**Positive notes:** all §15-§22 pointers resolve; §20 roles + cost-tiers match DEC-003 verbatim; §15 mode-gating consistent with WORKFLOW §5; §21 confidence thresholds + the can't-auto-evolve boundary (hard-refusals/citations/principles/rules) match DEC-003 Clause 4 (correct LLM04 design); §18 identifies the right enforcement SHAPE (the ERR-014 fix extends it); M8 gate is genuinely shipped. §16/§22/§15-promote probed + clean.

### §3.18 Target 18 — `DECISIONS.md` (Audit completed 2026-05-28 — Build Step 6; foundational doc, ALL FOUR agents)

**Audit context:** WS4 Build Step 6 Target 18 (commit 20/22). Decision H all-four-agents, focused lenses. The source-of-truth ADR record the entire framework cross-references (~81 refs to DEC-003 alone). Red-team flagged it as low-adversarial-surface (static prose ADR record, no runtime control behavior) → thin result as predicted. **0 Critical, 0 High → no ERROR-LOG entries** (consistent with a prose decision record).

**Activity logs:** code-reviewer `.tgf/state/agent-activity/code-reviewer/220ad2e4-bd51-47c2-8be0-07b08d705455.json`; security-auditor `.tgf/state/agent-activity/security-auditor/9a1dfcbe-63ca-4a89-a7be-a02eb4428ba1.json`; red-team `.tgf/state/agent-activity/red-team/70cb36cf-d714-4ab1-8893-52ccc33b8b55.json`; holistic `.tgf/state/agent-activity/holistic-reviewer/1175de3d-02b5-4c97-8e19-49f97cf8cfc1.json`

**Routed here (Medium/Low) — 3 Medium + 6 Low:**

| ID | Severity | Target | Description | Remediation hint | Priority hint |
|---|---|---|---|---|---|
| F-SA-DEC-01 | Medium | DECISIONS.md DEC-2026-05-20-010 Cl.3-4 + Consequences | DEC-010 (disable security-guidance plugin hook for TGF) rests on defense-substitution — four-pass review "covers the same risk surface" + becomes "the sole security-tooling layer." But **open ERR-2026-05-25-002** records the four agents' platform-level `tools:` restriction is NOT empirically validated (doc+persona-backed only). Asserts a substitute control as mature while a foundational property of its enforcement layer is an open ERR. **Same overstatement as F-SA-ARCH-01 — drift in TWO foundational docs.** | Add a maturity caveat to DEC-010 Consequences pointing to open ERR-2026-05-25-002; don't edit the substantive (defensible) decision. Re-close once ERR-002 resolves. | overstated security maturity (cross-doc w/ F-SA-ARCH-01) |
| F-HR-DEC-01 | Medium | DECISIONS.md DEC-2026-05-17-003 (~L541), DEC-2026-05-17-005 (~L404) | The framework's own **CP-8** prescribes a one-line FORWARD-pointer note on an amended ADR so a reader landing on the original sees it was superseded. DECISIONS.md gets the BACKWARD half right (every amending ADR cites what it amends) but NO amended ADR carries the forward marker (grep for the CP-8 marker = 0 hits). The source-of-truth ADR record fails to apply the framework's own continuity discipline to itself. | Add the CP-8 forward-note to each superseded ADR (DEC-003: Cl.1 amended by DEC-007, Cl.2 by DEC-005+DEC-006; DEC-005: physical-layout amended by DEC-009. "Original preserved."). One line each. | self-application gap (CP-8) |
| F-HR-DEC-02 | Medium | DECISIONS.md DEC-2026-05-26-011 L31 | DEC-011 L31 cites **`ERR-2026-05-26-005`** as where the deferred executable hook-side check is tracked. That ERR ID is **fabricated + never existed** — ERROR-LOG.md's own ERR-2026-05-27-005 context (L273) documents that ID as invented by an orchestrator dispatch. Real entry = **ERR-2026-05-27-005**. Source-of-truth ADR carries a dangling pointer to a non-existent log entry — the exact failure that turns a deferred item into a lost one. (DEC-011's other ERR ref, ERR-2026-05-25-003 at L37, is correct.) | Edit DEC-011 L31 → `(see ERR-2026-05-27-005)`. One-token fix, high-leverage. | dangling/fabricated cross-ref |
| F-SA-DEC-02 | Low | DECISIONS.md DEC-2026-05-17-004 | DEC-004 (six-clause citation discipline) correctly describes the paywalled (Cl.5) + fetched-untrusted (Cl.3) controls the later M9/M5 mitigations operationalize, but does NOT name an "M9 control" or "candidate-citation control" — those vocabularies live in RESEARCH-SECURITY.md §4.1 + red-team.md, authored AFTER DEC-004. Reader on DEC-004 can't discover the M9/M15/candidate machinery. Internal-completeness gap. | Add a forward-pointer in DEC-004 Consequences to RESEARCH-SECURITY.md as the operational implementation of Cl.1-4. | discoverability (predates impl) |
| F-RT-DEC-01 | Low | DECISIONS.md DEC-2026-05-26-011 (param clauses); DEC-2026-05-17-003 Cl.4 | AP-8 supersede-not-edit is enforced at ADR-HEADER granularity (satisfied — positive note) but does NOT reach silent in-place edits to control-locking PARAMETERS inside a Decision clause (DEC-011 crypto floors; DEC-003 Cl.4 "cannot auto-evolve"). Adversary w/ repo write can relax a sub-clause param undetectably. PRECONDITION: write access → Low. | DECISIONS-scoped instantiation of ERR-2026-05-28-014 / F-RT-CLAUDE-03; extend Phase-12 tamper-evidence scope to DECISIONS.md control-locking clauses. Add a one-line note to DEC-003 Cl.4 that "cannot auto-evolve" constrains the proposal workflow, NOT direct edits. No new ERR. | sub-clause tamper-evidence (under ERR-014) |
| F-RT-DEC-02 | Low | DECISIONS.md DEC-2026-05-17-003 Cl.3 | DEC-003 Cl.3 is the ARCHITECTURAL RATIFICATION of ERR-013's assumption: locks "cost-aware orchestration scales by change tier: Trivial = no subagents…" with NO requirement that tier be validated against the diff. Mislabel Large→Trivial = spec-correct zero-review path, blessed by the record. | No new remediation beyond ERR-013 (tier-floor hook) + ERR-014 (dispatch-floor). When that lands, amend DEC-003 Cl.3 via a NEW superseding ADR: "tier is a validated input, not asserted." Surfaced as the on-record ratification point. | ratification point (under ERR-013) |
| F-HR-DEC-03 | Low | DECISIONS.md — WORKFLOW.md bare L103/129/284/301/355 vs docs/WORKFLOW.md L60/83; ARCHITECTURE.md always bare | Within DECISIONS.md the docs/-resident files are referenced inconsistently (WORKFLOW.md bare 6× + docs/WORKFLOW.md 2×; ARCHITECTURE.md always bare despite living in docs/). **Inverse of the CLAUDE.md/ARCHITECTURE-L94 rot** (there a root file got a spurious docs/ prefix; here docs/ files lack it). Intra-prose mentions, not clickable links → low. | Fold into the F-HR-CLAUDE-01 canonical-location sweep; normalize (recommend bare names for prose, docs/ prefix only for backtick path-links). | path-prefix consistency (inverse) |
| F-CR-DEC-01 | Low | DECISIONS.md DEC-2026-05-17-003 (L501-541) | DEC-003 (most-cited ADR, ~81 refs) is MISSING the "Alternatives considered" section (Rule 5.2 names it required; the file's own template includes it). Reader can't see which interface designs were weighed + rejected. | Add an Alternatives section (or an honest "no formal alternatives — first-principles interface specs" note). | ADR-structure completeness |
| F-CR-DEC-02 | Low | DECISIONS.md DEC-2026-05-26-011 L33 | DEC-011 labels its final section "Downstream consequences:" vs the canonical "Consequences:" used by all 10 other ADRs + the template. Breaks any grep/linter keying on the literal header (incl. the ADR-structure linter DEC-011 itself contemplates). | Rename to "Consequences:". | header-consistency |
| F-CR-DEC-03 | Low | DECISIONS.md DEC IDs vs continuity rules.md L60 | **NNN is a GLOBAL monotonic counter** (001-011 across dates), NOT the within-day sequence Rule 5.2 documents ("NNN is sequence within the day, usually 001"). Practiced convention contradicts the stated rule → confuses next-ID assignment. | Align: (a) update Rule 5.2 to "global monotonic" (matches practice, leaves the ~11 widely-cited IDs untouched — **preferred**), or (b) renumber future IDs per-day. | convention-vs-rule drift |
| F-CR-DEC-04 | Low | DECISIONS.md DEC-001 (L460) + DEC-002 (L479) below DEC-003/004/005 | Within the 2026-05-17 cohort the file isn't strict newest-first by NNN (005,004 then 001,002,003). Cross-date ordering correct; only the intra-05-17 block out of descending order. Immutable foundational entries. | Optional: reorder the 05-17 block descending. Acceptable to waive. | intra-day ordering (waivable) |

**docs/-path-rot watch result (Target 18):** DECISIONS.md does NOT carry the docs/-prefix-on-root-artifact rot (its `docs/WORKFLOW.md` refs are CORRECT — WORKFLOW.md lives in docs/; the `docs/ERROR-LOG.md` at L64 is an intentional rhetorical "TGF doesn't have docs/ERROR-LOG.md"). A milder INVERSE inconsistency (docs/ files referenced bare) = F-HR-DEC-03 (Low). **Net: path-rot CONFIRMED not systemic** — limited to CLAUDE.md §7/§11 + ARCHITECTURE.md L94. One target (RESEARCH-SECURITY.md, Target 19) left to re-confirm.

**Cross-doc reproductions surfaced:** F-SA-DEC-01 is the DECISIONS.md-resident instance of the **four-pass-maturity overstatement** also flagged at ARCHITECTURE.md §20 (F-SA-ARCH-01) — now confirmed in 2 foundational docs, both gated on open ERR-2026-05-25-002. F-RT-DEC-01/02 are narrow on-record-ratification instantiations of the already-routed ERR-014/ERR-013 (no new exposure, no manufactured severity).

**Positive notes:** **amendment-chain integrity EXEMPLARY** — every amendment is a NEW superseding ADR with an explicit amendment note, never in-place; "edit in place" is enumerated + rejected in DEC-009 (L123) + DEC-005 (L396); textbook AP-8 / Rule 5.2 (the most important thing an ADR log can get right). DEC existence cross-check passed (all 11 cited ADRs exist; the 5 DEC-2026-02/03-* IDs are illustrative fixtures in anti-patterns.md → zero dangling DEC citations). The 3-part preload authority (DEC-007 Cl.5 + CLAUDE.md §6 + WORKFLOW.md §4/SKILL-INDEX) is mutually consistent, and agents/tgf-orchestrator.md matches DEC-007's literal `skills:` block. **CORRECTION to the dispatch framing recorded here:** DEC-007 amends DEC-003 **Clause 1** (not DEC-005 Cl.2); DEC-009 (not 007) amends DEC-005 — the FILE is internally correct; the prompt/memory summary was off (logged so future audits don't re-flag).

### §3.19 Target 19 — `docs/RESEARCH-SECURITY.md` (Audit completed 2026-05-28 — Build Step 6 CLOSED; foundational doc, ALL FOUR agents — LAST per-target audit, 19 of 19)

**Audit context:** WS4 Build Step 6 Target 19 (commit 21/22, **closes Build Step 6 + the per-target audit pass**). Decision H all-four-agents — genuinely applicable here (unlike DECISIONS.md's low-adversarial-surface): this is the framework's OWN information-supply-chain defense spec (M1-M19), LIVE this session (SessionStart announces it). The RICHEST target of the audit — all four agents had real surface and all four ran real general-purpose dispatches (not orchestrator-authored) verified against the live hook implementation + `.tgf/state/`. **0 Critical, 0 High → no ERROR-LOG entries** (max severity any of the four agents assigned was Medium; the orchestrator did NOT elevate against the dispatched personas' calibrated severity — see headline note).

**Activity logs:** code-reviewer `.tgf/state/agent-activity/code-reviewer/7b3e1a90-2c4d-4e8f-9a1b-3c5d7e9f0a2b.json`; security-auditor `.tgf/state/agent-activity/security-auditor/8c4f2b01-3d5e-4f90-ab2c-4d6e8f0a1b3c.json`; red-team `.tgf/state/agent-activity/red-team/9d5a3c12-4e6f-5a01-bc3d-5e7f9a1b2c4d.json`; holistic `.tgf/state/agent-activity/holistic-reviewer/ae6b4d23-5f70-6b12-cd4e-6f8a0b2c3d5e.json`

**⭐ HEADLINE — #1 WS5 elevation candidate (F-RT-RS-01 / F-HR-RS-05, found independently by red-team AND holistic):** the M1-M19 **write/commit enforcement perimeter is `skills/**`-only.** Verified in the live impl: `hook_research_pretool_write.py` gates on `_is_under_skills()` + `_is_markdown()`; the git pre-commit `SKILL_MD_RE` is anchored `^skills/`. So a flagged/unverified/spoofed citation incorporated into a `docs/*.md` foundational doc, an `agents/*.md` persona, or `CLAUDE.md` enters the framework with **NO PreToolUse-Write block and NO pre-commit citation check.** Agent personas are the highest-leverage position (the four review agents preload + trust them). Red-team frames it as an enforcement-coverage gap (Medium); holistic frames it as a conceptual self-exemption — the spec defining the supply-chain defense is itself outside that defense (Low). **Genuinely novel — red-team confirmed it is NOT captured by ERR-010..014.** Routed here at Medium (respecting both agents' calibrated severity — neither rated it High; the precondition of poisoning an authoritative source is the same high bar as the whole threat model), but flagged as the top elevation candidate for WS5 triage and cross-referenced to the ERR-014 enforcement-floor family as a distinct facet. WS5 must make a conscious decision: broaden the citation check to `docs/**`/`agents/**`/`CLAUDE.md`, OR explicitly document the `skills/**`-only boundary in §1 Scope as accepted residual.

**Routed here (Medium/Low) — 4 Medium + ~8 Low, deduplicated across the four agents:**

| ID | Severity | Target | Description | Remediation hint | Priority hint |
|---|---|---|---|---|---|
| F-RT-RS-01 / F-HR-RS-05 | **Medium** | RESEARCH-SECURITY.md §1 L13, §5.1 L169/177, §7.4 L370 (+ live hooks) | **Perimeter scope = `skills/**`-only** (headline above). Write/commit citation enforcement does not cover docs/**, agents/**, CLAUDE.md; this doc is itself a docs/ file so its own §9 citations are unenforced. Defensible scoping but undocumented in §1. | Broaden the PreToolUse-Write/pre-commit check to all citation-bearing artifacts, OR add one sentence to §1 "What this doesn't cover" stating the skills/**-only boundary + that foundational docs follow §9 + §10.5 instead. **#1 WS5 elevation candidate.** | enforcement-perimeter (novel; ERR-014 family, distinct facet) |
| F-SA-RS-01 | **Medium** | RESEARCH-SECURITY.md §6 L302/306, §7.1 steps 2+12, §10.1 L484 | **Defense-in-theater:** M6 (change-history) + M10 (citation-existence) are presented as active bold "Layer 1 (hook)" enforcement + §7.1 blocking/checking steps, but NEITHER is implemented (no m6_*/m10_* helper, empty `citation-indexes/`, no smoke test). M17 — equally unimplemented — IS honestly disclaimed 3×; M6/M10 are not. A reviewer relying on the doc believes Persona-2/4 watering-hole (M6) + Persona-3 authority-spoofing (M10) are mechanically caught — they're undefended at Layer 1. | Mark M6+M10 with the same "not yet implemented" qualifier M17 gets (§6, §7.1, §548 status note), or move to §8 Residual Risk. Don't represent as active bold-Layer-1 until helpers + smoke tests exist. **#2 elevation candidate.** | doc overstates live control surface |
| F-CR-RS-01 / F-SA-RS-02 / F-HR-RS-03 / F-RT-RS-05 | **Medium** | RESEARCH-SECURITY.md §5.1 L209-211 + §5.2 | **Stale as-built inventory (aggregate, all 4 agents):** "29 sources"→**44** actual; "8 schemas (one per source type)"→**15**; "5 publishers (OWASP/NIST/IETF/ISO/MITRE)"→**7 orgs** (missing CIS + Microsoft — BOTH now load-bearing: §9 itself cites Microsoft Spotlighting, WORKFLOW-V2 cites CIS Controls v8.1). Security angle (security-auditor): the M15 allow-list + M12 independence map are larger/broader than documented; a reviewer under-enumerates the fetch surface by 15 sources + wouldn't expect a vendor-tier Microsoft source in M5/M12 corroboration. Layer-5 angle (red-team): degrades the human-review the doc designates as the final defense — the reviewer can't catch what the spec hides. | Refresh §5.1/§5.2 to 44/15/7 (incl. CIS+Microsoft + note Microsoft's vendor tier), OR replace hard counts with a pointer to live state + "counts as of <date>". Consider auto-deriving the inventory from the registry/org-map/schema dir. | stale inventory (froze at WS1, never tracked WS2-4 growth) |
| F-CR-RS-04 / F-HR-RS-01 | **Medium** | RESEARCH-SECURITY.md §7.5 L389 (+ agents/security-auditor.md) | §7.5 "security-auditor.md includes RESEARCH-SECURITY.md in its preloaded materials" is **false as written** — the `skills:` frontmatter loads skill DIRECTORIES; a docs/ file is structurally ineligible. It's referenced only in PROSE for the M8 gate. "preloaded" overstates the guarantee; the "eventually" hedge misrepresents a WS3-completed deliverable. (code-reviewer Low / holistic Medium → carried Medium for the fuller mechanism analysis.) | Reword to "references RESEARCH-SECURITY §5.5/§7 in persona prose; does NOT preload via skills:". If guaranteed-load is desired → a separate work-package (a research-security skill dir, or explicit context injection); track WS5. | calibrated-confidence (claims a preload that can't exist) |
| F-CR-RS-02 / F-SA-RS-04 | Low | RESEARCH-SECURITY.md §8.3 L424, §10.4 L520 | Script-naming inconsistency: §8.3/§10.4 reference `m4-pattern-scan.py` / `m13-hash-check.py` (hyphens, + `-scan` vs real `-detect`); actual files `m4_pattern_detect.py` / `m13_hash_check.py`. A maintainer greps a non-existent filename + doubts the code is built. | Reconcile §8.3/§10.4 to the canonical underscore names matching §5.1 + disk. | as-built name fidelity |
| F-CR-RS-03 | Low | RESEARCH-SECURITY.md §6 L317 vs table L296-315 + §5.1 L169 | Self-contradicting count: L317 "Eleven of nineteen are primarily Layer 1" but the §6 table bolds only **9** primary-Layer-1 rows + §5.1 L169 enumerates only **7** content-analysis hooks. Three counts (7/9/11) unreconciled; the "most discipline is now mechanical" headline rests on a number that doesn't match the table. | Pick the authoritative count (9 bolded reads most defensible) + make L317 match, or state the inclusion rule. | internal-consistency |
| F-CR-RS-05 | Low | RESEARCH-SECURITY.md §5.1 table vs §5.2 L235 + §7.1 L345 | `.tgf/state/citation-indexes/` (M10 state dir) is described in §5.2 + referenced in §7.1 + exists on disk, but is ABSENT from the §5.1 "complete" as-built inventory table (which lists every other state path). | Add an `L2 state | .tgf/state/citation-indexes/ | (lazy) cached doc IDs for M10` row. | inventory completeness |
| F-CR-RS-06 / F-HR-RS-02 | Low | RESEARCH-SECURITY.md L3 + L542 | **docs/-path-rot:** L3 + L542 reference `docs/DECISIONS.md`; DECISIONS.md is at ROOT. Third foundational doc with the rot (see §4 systemic-pattern update). DEC-IDs themselves resolve; docs/ARCHITECTURE.md + docs/WORKFLOW.md refs ARE correct — selective over-extension. | Change both to bare `DECISIONS.md`. Fold into the single systemic WS5 path-correction sweep. | path-prefix (3rd instance → systemic) |
| F-SA-RS-03 | Low | RESEARCH-SECURITY.md §5.5 L286, §7.3 L364 | §5.5 renders an interactive "Proceed? [y/N]" + §7.3 says the Stop hook "surfaces summary to human", but actual M8 enforcement is **artifact-presence** (`m8-approvals/*.json` checked by Stop + pre-commit), not interactive stdin. Mechanism is SOUND + arguably stronger; defect is presentational — a maintainer might "fix" a non-existent prompt. | Add one line: the "[y/N]" illustrates the verification SUMMARY a human evaluates out-of-band; enforcement is artifact-presence, not an interactive prompt. | presentational clarity |
| F-RT-RS-02 | Low | RESEARCH-SECURITY.md §8.4 (+ SessionStart hook) | §8.4 claims state corruption is "mitigated by state file integrity checks at SessionStart", but the SessionStart hook does only **file-existence** checks. State is gitignored + writable → an attacker with repo-write forges a `{status:'verified'}` research-log or a covers_files-matching m8-approval, trusted with zero integrity validation. Downgrades an exploitable-with-repo-write gap to "accepted residual" on a control that doesn't exist as described. | Implement the claimed check (signed/hash-chained records or a tamper-evident manifest), OR correct §8.4 to state integrity is existence-only + forged 'verified' is accepted residual under repo-write. | doc-vs-impl drift (repo-write precond) |
| F-RT-RS-03 | Low | RESEARCH-SECURITY.md §7.6 L406 | The §7.6 pre-commit backstop has 3 undocumented fail-open paths: (a) not installed on a fresh clone (the `.git/hooks/pre-commit` symlink isn't version-controlled); (b) `TGF_PRECOMMIT_BYPASS` env + `git --no-verify`; (c) missing `.claude/hooks/lib` → wrapper warns + exit 0 (fail-open by design). | Surface the 3 paths in §7.6/§8 as known residuals; consider a server-side CI check as the real backstop (client git hooks are bypassable + non-portable). | fail-open enumeration |
| F-RT-RS-04 | Low | RESEARCH-SECURITY.md §5.3 L251 | §5.3 "Combined with Layer 1 hooks... it is sufficient" overstates: M9 has NO mechanical floor (persona+human only), AND Layer-1 hooks fail OPEN on malformed stdin (`common.py` read_input→{}→passthrough; load_json→default). A malformed/oversized payload makes PreToolUse-Write + Stop pass through silently. | Soften §5.3 to reflect M9 is persona+human-only with no mechanical floor + Layer-1 hooks fail open on malformed input; cross-ref §8.7 honestly. | overstated sufficiency |
| F-HR-RS-04 | Low | RESEARCH-SECURITY.md §5.1 L184 (+ ARCHITECTURE §18, DEC-2026-05-19-009) | **Hook-layout drift:** §5.1 describes the canonical layout as `.claude/hooks/` citing only DEC-005; DEC-2026-05-19-009 later amended canonical layout to plugin-native `hooks/hooks.json` (ARCHITECTURE §18 was amended to match). RESEARCH-SECURITY never references DEC-009. As-built is split: real hooks at `.claude/hooks/` (matches doc) but a plugin-native `hooks/hooks.json` exists at root as an empty 18-byte stub — both doc-staleness AND incomplete plugin migration. | Add a §5.1 note acknowledging DEC-009 (.claude/hooks/ = WS1/standalone form; plugin distribution uses hooks/hooks.json, currently a stub). Track the actual migration in WS5/Phase-12. | DEC-superseded layout (+ incomplete migration) |

**M9 canonical relationship CONFIRMED (closes the F-SA-ARCH-02 loop):** RESEARCH-SECURITY is the complete, canonical home for M9 (§4.1 def, §3.5 Persona-5 threat model, §5.3 persona rule "one source of evidence, not two", §5.5 memory-alignment flag, §6 map, §8.7 residual). ARCHITECTURE §17 contains ZERO M9 (grep clean). WORKFLOW.md §3/§7.3 DO carry M9 correctly. So the omission is specific to ARCHITECTURE §17 — **§17 is the incomplete doc; its WS5 fix should cross-ref RESEARCH-SECURITY §4.1/§8.7**, not invent its own M9 text.

**docs/-path-rot RE-CONFIRMED + REVISED TO SYSTEMIC (holistic, owns the synthesis):** with RESEARCH-SECURITY making it **3 of 5 foundational docs** (CLAUDE.md §7/§11 = all 5 root artifacts; ARCHITECTURE.md L94 = docs/DECISIONS.md; RESEARCH-SECURITY L3+L542 = docs/DECISIONS.md), and WORKFLOW.md the clean counter-example (18 bare-correct `DECISIONS.md` refs) + DECISIONS.md self-consistent, **Target 18's tentative "not systemic" conclusion is OVERTURNED.** It is a systemic Build-Step-6-era pattern (a shared incorrect assumption that root governance artifacts live under docs/) → one systemic WS5 path-correction sweep, NOT per-doc one-offs. See §4 update below.
